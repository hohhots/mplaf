 ...
 * the Java<sup><font size="-2">TM</font></sup> look and feel (Mongol).
 * These icons are used extensively in Mongol via the defaults mechanism.
 ...
 */
public class MongolIconFactory implements Serializable {
    ...
    public static Icon getHorizontalSliderThumbIcon() {
        if (MongolLookAndFeel.usingOcean()) {
    ...
    private static class FileChooserDetailViewIcon implements Icon, UIResource, Serializable {
	    ...
	    g.setColor(MongolLookAndFeel.getPrimaryControlInfo());
	    //     top
	    ...
	    g.setColor(MongolLookAndFeel.getPrimaryControl());
	    g.drawRect(3,3, 2,3);   // top
	    g.drawRect(3,11, 2,3);  // bottom

	    // Draw inner inner highlight on documents
	    g.setColor(MongolLookAndFeel.getPrimaryControlHighlight());
    ...
    private static class FileChooserHomeFolderIcon implements Icon, UIResource, Serializable {
	    ...
	    // Draw roof body
    	    g.setColor(MongolLookAndFeel.getControlDarkShadow());
    ...
    private static class FileChooserNewFolderIcon implements Icon, UIResource, Serializable {
	    ...
	    // Draw tab on folder
	    g.setColor(MongolLookAndFeel.getPrimaryControlDarkShadow());
    ...
    public static class PaletteCloseIcon implements Icon, UIResource, Serializable{
	    ...
	    Color back;
	    Color highlight = MongolLookAndFeel.getPrimaryControlHighlight();
	    Color shadow = MongolLookAndFeel.getPrimaryControlInfo();
	    if (buttonModel.isPressed() && buttonModel.isArmed()) {
	        back = shadow;
	    } else {
	        back = MongolLookAndFeel.getPrimaryControlDarkShadow();
	    }
	...
	int iconSize = 16;
	    ...
	    Color backgroundColor = MongolLookAndFeel.getPrimaryControl();
	    Color internalBackgroundColor = 
		MongolLookAndFeel.getPrimaryControl();
	    Color mainItemColor = 
		MongolLookAndFeel.getPrimaryControlDarkShadow();
	    Color darkHighlightColor = MongolLookAndFeel.getBlack();
	    Color xLightHighlightColor = MongolLookAndFeel.getWhite();
	    Color boxLightHighlightColor = MongolLookAndFeel.getWhite();
	    
	    // if the inactive window
	    if (parentButton.getClientProperty("paintActive") != Boolean.TRUE)
	    {
		backgroundColor = MongolLookAndFeel.getControl();
		internalBackgroundColor = backgroundColor;
		mainItemColor = MongolLookAndFeel.getControlDarkShadow();
		// if inactive and pressed
		if (buttonModel.isPressed() && buttonModel.isArmed()) {
		    internalBackgroundColor =
			MongolLookAndFeel.getControlShadow();
		...
		internalBackgroundColor =
		    MongolLookAndFeel.getPrimaryControlShadow();
		xLightHighlightColor = internalBackgroundColor;
		mainItemColor = darkHighlightColor;
	...
	int iconSize = 16;
	    ...
	    Color ulLightHighlightColor = MongolLookAndFeel.getWhite();
	    Color lrLightHighlightColor = MongolLookAndFeel.getWhite();
    ...
    private static class InternalFrameDefaultMenuIcon implements Icon, UIResource, Serializable {
	    ...
	    Color windowBodyColor = MongolLookAndFeel.getWindowBackground();
	    Color titleColor = MongolLookAndFeel.getPrimaryControl();
	    Color edgeColor = MongolLookAndFeel.getPrimaryControlDarkShadow();
	...
	private void paintOceanIcon(Component c, Graphics g, int x, int y) {
		...
		if (model.isPressed() && model.isArmed()) {
                    g.setColor(MongolLookAndFeel.getControlShadow());
                    g.fillRect(0, 0, w, h);
                    g.setColor(MongolLookAndFeel.getControlDarkShadow());
                    g.fillRect(0, 0, w, 2);
                    ...
                    MongolUtils.drawGradient(c, g, "CheckBox.gradient", 0, 0,
                                            w, h, true);
                    g.setColor(MongolLookAndFeel.getControlDarkShadow());
		...
		g.setColor( MongolLookAndFeel.getControlInfo() );
	    } else {
                g.setColor(MongolLookAndFeel.getControlDarkShadow());
                g.drawRect(0, 0, w - 1, h - 1);
	    }
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
		...
		if (model.isPressed() && model.isArmed()) {
		    ...
		    MongolUtils.drawPressed3DBorder(g, x, y, controlSize, controlSize);
		} else {
		    MongolUtils.drawFlush3DBorder(g, x, y, controlSize, controlSize);
		}
    ...
    private static class RadioButtonIcon implements Icon, UIResource, Serializable {
	    ...
	    g.translate(x, y);
            if (enabled && !pressed) {
                ...
                MongolUtils.drawGradient(c, g, "RadioButton.gradient",
                                        1, 1, 10, 10, true);
	    ...
	    if (!enabled) {
                g.setColor(MongolLookAndFeel.getInactiveControlTextColor());
	    }
	    else {
                g.setColor(MongolLookAndFeel.getControlDarkShadow());
	    }
	    ...
	    Color shadow = MongolLookAndFeel.getControlShadow();
	    Color darkCircle = MongolLookAndFeel.getControlDarkShadow();
	    Color whiteInnerLeftArc = MongolLookAndFeel.getControlHighlight();
	    Color whiteOuterRightArc = MongolLookAndFeel.getControlHighlight();
    ...
    private static class TreeHardDriveIcon implements Icon, UIResource, Serializable {
	    ...
	    g.setColor(MongolLookAndFeel.getControlHighlight());
	    //     top disk
	    g.drawLine(4,3, 5,3);
	    g.drawLine(7,3, 9,3);
    ...
    private static class TreeFloppyDriveIcon implements Icon, UIResource, Serializable {
	    ...
	    g.setColor(MongolLookAndFeel.getControlDarkShadow());
	    g.fillRect(5,2, 6,5); // Mongol disk protector part
	    ...
	    // Draw text on label
	    g.setColor(MongolLookAndFeel.getPrimaryControlShadow());
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
		...
		// Draw fill
		g.setColor( MongolLookAndFeel.getWindowBackground() );
		g.fillRect( 4, 2, 9, 12 );
	...
	public void paintIcon(Component c, Graphics g, int x, int y) {
	    ...
	    if (image == null || cachedOrientation != MongolUtils.isLeftToRight(c)) {
		cachedOrientation = MongolUtils.isLeftToRight(c);
		if (gc != null) {
		    image = gc.createCompatibleImage(getIconWidth(), 
						     getIconHeight(),
						     Transparency.BITMASK);
	    ...
	    if (MongolUtils.isLeftToRight(c)) {
	...
	public void paintMe(Component c, Graphics g, int x, int y) {
	    ...
	    int xoff = (MongolUtils.isLeftToRight(c)) ? 0 : 4;
  ...
  //
	...
	public void paintIcon( Component c, Graphics g, int x, int y )
	{
	    ...
	    if ( !model.isEnabled() )
	    {
	        g.setColor( MongolLookAndFeel.getMenuDisabledForeground() );
	    }
	    else
	    {
	        if ( model.isArmed() || ( c instanceof JMenu && model.isSelected() ) )
		{
		    g.setColor( MongolLookAndFeel.getMenuSelectedForeground() );
		}
		else
		{
            ...
            if( MongolUtils.isLeftToRight(b) ) {
                g.drawLine( 0, 0, 0, 7 );
                g.drawLine( 1, 1, 1, 6 );
                g.drawLine( 2, 2, 2, 5 );
                g.drawLine( 3, 3, 3, 4 );
	...
	public void paintOceanIcon(Component c, Graphics g, int x, int y) {
	    ...
	    g.translate(x, y);
            if (isEnabled) {
                MongolUtils.drawGradient(c, g, "CheckBoxMenuItem.gradient",
                                        1, 1, 7, 7, true);
	    ...
	    if (isSelected) {
		    ...
		    if (isArmed || ( c instanceof JMenu && isSelected)) {
		        g.setColor(
                            MongolLookAndFeel.getMenuSelectedForeground() );
		    }
		    else {
                         g.setColor(MongolLookAndFeel.getControlInfo());
		    }
	...
	public void paintOceanIcon(Component c, Graphics g, int x, int y) {
	    ...
	    g.translate( x, y );
            ...
            if (isEnabled) {
                MongolUtils.drawGradient(c, g, "RadioButtonMenuItem.gradient",
                                        1, 1, 7, 7, true);
	        if (isPressed || isArmed) {
		    g.setColor(MongolLookAndFeel.getPrimaryControl());
                }
...
private static class VerticalSliderThumbIcon implements Icon, Serializable, UIResource {
	...
	boolean leftToRight = MongolUtils.isLeftToRight(slider);
	...
	if ( slider.hasFocus() ) {
	    ...
	    g.setColor( slider.isEnabled() ? MongolLookAndFeel.getPrimaryControlInfo() :
			                     MongolLookAndFeel.getControlDarkShadow() );
	}
	...
	// Draw the highlight
	if ( slider.isEnabled() ) {
	    g.setColor( slider.hasFocus() ? MongolLookAndFeel.getPrimaryControl()
			: MongolLookAndFeel.getControlHighlight() );
	    if (leftToRight) {
	        g.drawLine( 1, 1, 8, 1 );
		g.drawLine( 1, 1, 1, 13 );
	    }
    ...
    private static class OceanVerticalSliderThumbIcon extends CachedPainter
        ...
        public void paintIcon(Component c, Graphics g, int x, int y) {
            ...
            paint(c, g, x, y, getIconWidth(), getIconHeight(),
                  MongolUtils.isLeftToRight(c), c.hasFocus(), c.isEnabled(),
                  MongolLookAndFeel.getCurrentTheme());
        }
        ...
        protected void paintToImage(Component c, Image image, Graphics g2,
            ...
            if (!enabled) {
                ...
                MongolUtils.drawGradient(c, g, "Slider.focusGradient",
                                        1, 1, 14, 14, false);
            }
            else {
                MongolUtils.drawGradient(c, g, "Slider.gradient",
                                        1, 1, 14, 14, false);
            }
            g.setClip(clip);
            ...
            if (hasFocus) {
                ...
                g.setColor(enabled ? MongolLookAndFeel.getPrimaryControlInfo() :
                           MongolLookAndFeel.getControlDarkShadow());
            }
